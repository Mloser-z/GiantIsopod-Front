<template>
 <div>
   ...
   <div id="listBox">
      <div v-for="item in Result_list" :key="item.name_en" class="singleBox">
        <div class="upBox">
          <img :src="item.url" @click="postSearch(item.name_en)" title="查看详情">
          <p1 @click="postSearch(item.name_en)">{{item.name_en}}</p1>
          <p2 @click="postSearch(item.name_en)">中文名：{{item.name_zh}}</p2>
        </div>
        <div class="downBox">
          {{item.info}}
        </div>
...
const postSearch = (name_en) => {
  let params = { name_en: name_en ,from: '/key-search'};
